...
\section{Introduction}
	Air traffic flow management (ATFM) refers to strategic air traffic control where the goal is to match the demand for air transportation system resources, including airport arrival and departure `slots' and space in sections of airspace, with the available supply looking several hours into the future.  In ATFM, one of the most commonly used strategies is the Ground Delay Program (GDP).  During a GDP in the United States, Federal Aviation Administration (FAA) air traffic managers will ask air carriers to delay flights destined for a capacity constrained destination airport before the flights have taken off from their origin airports.  When announcing the GDP, traffic managers will set the {\bf program rate}, detailing how many arriving flights the constrained airport can accommodate, the {\bf departure scope}, describing which flights will be delayed, and other operational details.  Note that, according to the departure scope, long-distance flights are often {\bf exempted} from a GDP and are not delayed while other flights are delayed.\\
	\newline
	Numerous research articles have been published on the topic of GDP optimization. \cite{odoni} described the problem in detail some 30 years ago. \cite{terrab} developed an optimal dynamic programming algorithm and various heuristic algorithms for solving deterministic and stochastic versions of what the authors call the Ground Holding Problem. \cite{ssp} introduce a mathematical program that captures airport and airspace capacity constraints and that has a special structure allowing the program to be solved to optimality when provided with realistic-sized problems.\\
	\newline
	...
	\newline
	\textcolor{red}{I added two paragraphs above on GDP optimization and airport capacity estimation/prediction.  We should probably write more about GDP optimization - and cite more papers.  But it's hard to tell which papers are important.  I actually think the old papers I found are really good - maybe better than the newer papers by Mukherjee and others that I thought I would cite.  You want to write more on GDP optimization and I can write more on weather forecasts and airport capacity prediction? -Ken}\\
	\newline
