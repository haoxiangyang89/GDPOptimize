...
\section{Solution Methods}
	...
	\newline
	We take another approach to solve this problem, combining our modeling effort with state-of-art decomposition tools which have been designed to deal with problems with a modest number of scenarios per time period but a long time horizon. Stochastic dual dynamic program (SDDP) was developed by \cite{pereira1991multi} for a multistage stochastic linear program with exogenous information that is interstage-independent \textcolor{red}{(this needs a bit more clarification)}. The algorithm uses the piecewise linear nature of the cost-to-go function to iteratively generate Benders cuts to approximate it.\\
